...
{
    int hkey = Hash(ht, x.Maso);

    List *bucket = ht.table[hkey];
    if (bucket == nullptr)
    {
        ht.table[hkey] = new List();
        ht.table[hkey].head = new Node(x);
        ht.table[hkey].tail = ht.table[hkey]->head;
        ht.n++;
        return 1;
    }
